# 🤗Estrategias para conseguir Usuarios/Clientes

## 📝🤓Temas a tratar en la sesión

- [Sitio Web](#sitio-web)
- [Hospedaje Web](#hospedaje-web)
- [Comunidad](#comunidad)
- [Donaciones](#donaciones)
- [Ventas](#ventas)

<br><br><br><br><br><br><br><br><br><br>
<br><br><br><br><br><br><br><br><br><br>
<br><br><br><br><br><br><br><br><br><br>

## 🌐Sitio Web

- Crear tu propio sitio web y cómprate un dominio.
- Siempre compra por separado el nombre de tu dominio del hospedaje del sitio web.
- Puedes comenzar con un simple archivo HTML con tus datos y una foto...
- O puedes usar alguna [plantilla gratis](https://colorlib.com/wp/templates/) que se adapte a tus necesidades...
- O puedes usar [WordPress.org](https://wordpress.org/).

[🔙](#temas-a-tratar-en-la-sesión)

<br><br><br><br><br><br><br><br><br><br>
<br><br><br><br><br><br><br><br><br><br>
<br><br><br><br><br><br><br><br><br><br>

## 💻Hospedaje Web

- [NameCheap](https://www.namecheap.com/)
- [Neubox](https://neubox.com/)
- [DreamHost](https://www.dreamhost.com/)
- [HostGator](https://www.hostgator.mx/)
- [GoDaddy](https://mx.godaddy.com/)
- [Wix](https://es.wix.com/)
- [WordPress.com](https://es.wordpress.com/)
- [000webhost by Hostinger](https://www.000webhost.com/)
- [AwardSpace](https://www.awardspace.com/free-hosting/)

[🔙](#temas-a-tratar-en-la-sesión)

<br><br><br><br><br><br><br><br><br><br>
<br><br><br><br><br><br><br><br><br><br>
<br><br><br><br><br><br><br><br><br><br>

## 😍Comunidad

Herramientas para comenzar a crear comunidad:

- **Lista de Correos**: [TinyLetter](https://tinyletter.com/)
- **Formularios**: [FormSubmit](https://formsubmit.co/)
- **Compartir en Redes Sociales**: [AddThis](https://www.addthis.com/)
- **Chats en vivo**:
  - [Drift](https://www.drift.com/)
  - [Canales de Slack](https://slack.com/intl/es-mx/)
  - Grupos de WhatsApp
- **Encuestas**: [Google Forms](https://docs.google.com/forms/)
- **Comentarios**: [Disqus](https://disqus.com/)
- **Analíticas**: [Google Analytics](https://analytics.google.com/)
- **Pagos en Línea**: [Paypal](https://www.paypal.com/)

[🔙](#temas-a-tratar-en-la-sesión)

<br><br><br><br><br><br><br><br><br><br>
<br><br><br><br><br><br><br><br><br><br>
<br><br><br><br><br><br><br><br><br><br>

## ❤️Donaciones

- Cuenta bancaria: Depósitos directos o en establecimientos.
- [Western Union](https://www.westernunion.com/mx/es/home.html)
- [Paypal Me](https://www.paypal.me/)
- [Patreon](https://www.patreon.com/)
- [KickStarter](https://www.kickstarter.com/)
- [Ko-fi](https://ko-fi.com/)

[🔙](#temas-a-tratar-en-la-sesión)

<br><br><br><br><br><br><br><br><br><br>
<br><br><br><br><br><br><br><br><br><br>
<br><br><br><br><br><br><br><br><br><br>

## 🤑Ventas

- Ventas Directas
  - Cuenta bancaria
  - [Paypal](https://www.paypal.com/)
- Programas de Afiliados:
  - [Amazon](https://afiliados.amazon.com.mx/)
  - [Mercado Libre](https://listado.mercadolibre.com.mx/programa-de-afiliados)
  - [eBay](https://centroparavendedores.ebay.es/ebay-partner-network)
  - [AliExpress](https://portals.aliexpress.com/)
- Tiendas en línea
  - [Kichink](https://www.kichink.com/planes)
  - [Shopify](https://www.shopify.com.mx/precios)
  - [Teespring](https://teespring.com/)
- Enseña en línea
  - [Udemy](https://www.udemy.com/)
  - [Teachable](https://teachable.com/)
- Experiencias Personalizadas
  - [Google Negocios](https://www.google.com/intl/es-419_mx/business/)
  - [Airbnb](https://www.airbnb.mx/s/experiences)

[🔙](#temas-a-tratar-en-la-sesión)

<br><br><br><br><br><br><br><br><br><br>
<br><br><br><br><br><br><br><br><br><br>
<br><br><br><br><br><br><br><br><br><br>
